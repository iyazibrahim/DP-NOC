...
mkdir -p "$OUT_DIR"
if [[ -d "$OUT_FILE" ]]; then
  echo "ERROR: $OUT_FILE is a directory (Docker created it for a missing file bind). Remove the directory and Force apply." >&2
  exit 1
fi
if ! touch "$OUT_FILE" 2>/dev/null; then
  echo "ERROR: cannot write $OUT_FILE — is DATA_DIR a writable named volume (noc_sitebox_data)?" >&2
  exit 1
fi
...
prometheus.exporter.unix "nuc_host" {
  ...
  // ISP speedtest textfile (noc_speedtest_*.prom from speedtest service)
  textfile {
